...
try:
    ...
    class_algorithm = getattr(
        module_algorithm, sys.argv[3] if len(sys.argv) > 3 else "ExampleAlgorithm"
    )
    module_heuristic = __import__("heuristics")
    class_heuristic = getattr(
        module_heuristic, sys.argv[4] if len(sys.argv) > 4 else "ExampleHeuristic"
    )
